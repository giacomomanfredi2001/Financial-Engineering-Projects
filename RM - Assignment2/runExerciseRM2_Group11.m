...
% Group 11
% SW: Marchetto Erica
% Manfredi Giacomo, Maspes Marco, Meschieri Andrea
...
%      alongside with their market prices (dirty)
% 
...
%
%USES:
% function FV_risky_bond(cf_schedule, Q, ZC_curve, R) 
% function R_IRB(PD) 
...
bD = icdf('Normal', Q(1, 3), 0, 1);             % Barrier to Def.
bd = icdf('Normal', sum(Q(1, 2:3)), 0, 1);      % Barrier to down (HY)
b = 1.0e6;                                     % Barrier to up (never)
...
loss = L_D*scen_D;
VaR_DO = -quantile(loss, 1 - 0.999);

% Default and migration case (DM)
loss = L_D*scen_D + L_d*scen_d + L_i*scen_i;
VaR_DM = -quantile(loss, 1 - 0.999);

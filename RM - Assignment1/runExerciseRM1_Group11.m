...
%USES
% function PV_risky_bond_h(cf_schedule, h_curve, ZC_curve,R)
% function PV_risky_bond_Z(z, cf_schedule, ZC_curve)
...
% Computation of the Hazard Rates of IG

IG_h_curve = zeros(2, 2);
IG_h_curve(:, 1) = [1; 2];

IG_h_curve(1,2) = lsqnonlin(@(h) PV_risky_bond_h(IG_cf_schedule_1y, [1 h; 2 0], ZC_curve, R) - IG_Bond_dirty_price_1y, 0, 0);
IG_h_curve(2,2) = lsqnonlin(@(h) PV_risky_bond_h(IG_cf_schedule_2y, [1 IG_h_curve(1,2); 2 h], ZC_curve, R) - IG_Bond_dirty_price_2y, 0, 0);

% Computation of the Hazard Rates of HY

HY_h_curve = zeros(2, 2);
HY_h_curve(:, 1) = [1; 2];

HY_h_curve(1,2) = lsqnonlin(@(h) PV_risky_bond_h(HY_cf_schedule_1y, [1 h; 2 0], ZC_curve, R) - HY_Bond_dirty_price_1y, 0, 0);
HY_h_curve(2,2) = lsqnonlin(@(h) PV_risky_bond_h(HY_cf_schedule_2y, [1 HY_h_curve(1,2); 2 h], ZC_curve, R) - HY_Bond_dirty_price_2y, 0, 0);
...
% Computation of the different Z-spread
IG_z_1y = lsqnonlin(@(z) PV_risky_bond_Z(z, IG_cf_schedule_1y, ZC_curve) - IG_Bond_dirty_price_1y, 0, 0);
IG_z_2y = lsqnonlin(@(z) PV_risky_bond_Z(z, IG_cf_schedule_2y, ZC_curve) - IG_Bond_dirty_price_2y, 0, 0);
HY_z_1y = lsqnonlin(@(z) PV_risky_bond_Z(z, HY_cf_schedule_1y, ZC_curve) - HY_Bond_dirty_price_1y, 0, 0);
HY_z_2y = lsqnonlin(@(z) PV_risky_bond_Z(z, HY_cf_schedule_2y, ZC_curve) - HY_Bond_dirty_price_2y, 0, 0);
...
% We will write the Q transition probability matrix in the following way:
% Q = [ q11 q12 q13
%       q21 q22 q23
%       0   0   1   ]

% Computation of the default probabilities at 1y
q13 = 1 - exp(-IG_h_curve(1,2)*1);
q23 = 1 - exp(-HY_h_curve(1,2)*1);

% Computation of the default probabilities at 2y

q13_2y = 1 - exp(-IG_h_curve(1,2)*1)*exp(-IG_h_curve(2,2)*1);
q23_2y = 1 - exp(-HY_h_curve(1,2)*1)*exp(-HY_h_curve(2,2)*1);

% Creation of the matrixes to compute the probabilities. It's a linear
% system in 4 equations deriving from:
% - 1st: pIG = 1                      at first year
% - 2nd: pHY = 1                      at first year
% - 3rd: pIG_default = q13_2y         at second year
% - 4th: pHY_default = q23_2y         at second year

A = [1 1 0 0; 0 0 1 1; q13 q23 0 0; 0 0 q13 q23];
b = [1 - q13; 1 - q23; q13_2y - q13; q23_2y - q23];

% Solving of the Linear solution of the system: [q11 q12 q21 q22]
q_probs = linsolve(A,b);

Q = [q_probs(1) q_probs(2) q13; q_probs(3) q_probs(4) q23; 0 0 1];

format short;
disp('––– Part II Q1: Displaying of the transition matrix Q –––')
Q

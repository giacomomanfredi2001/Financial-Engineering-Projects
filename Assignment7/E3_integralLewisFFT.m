function [Z, integral] = E3_integralLewisFFT(M, dz, alpha, sigma, k, eta, timeToMaturity, flag_model)
...
% function LaplaceTransform(deltaT, k, alpha, w, sigma)
    ...
    xi = -X-1i/2;
    charFct = exp(-1i*xi*log(LaplaceTransform(timeToMaturity, k, alpha, eta, sigma, flag_model))) .* ...
        LaplaceTransform(timeToMaturity, k, alpha, (xi.^2+1i*(1+2*eta).*xi)/2, sigma, flag_model);
